#!/usr/bin/env tsx
/**
 * Database builder for Haiti Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Haiti Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Haitian statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('jurisdiction', 'HT');
    insertMeta.run('source', 'dfrn.gouv.ht (offline)');
    insertMeta.run('licence', 'Government Publication (offline portals)');
  });
  writeMeta();
