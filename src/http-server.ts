...
function resolvePackageJson(): string {
  // Handle both tsconfig layouts:
  //   rootDir=src   → __dirname is dist/, package.json is one up
  //   rootDir=null  → __dirname is dist/src/, package.json is two up
  for (const rel of ['..', '../..']) {
    const p = join(__dirname, rel, 'package.json');
    if (existsSync(p)) return p;
  }
  throw new Error('package.json not found relative to dist/');
}
const pkg = JSON.parse(readFileSync(resolvePackageJson(), 'utf-8'));
